...
fn main() -> Result<()> {
    ...
    bootloader::UefiBoot::new(AsRef::as_ref("target/x86_64-unknown-none/debug/test-kernel"))
    .create_disk_image(out_gpt_path)?;

    let out_bios_path = &AsRef::as_ref("target/bios.img");
    bootloader::BiosBoot::new(AsRef::as_ref("target/x86_64-unknown-none/debug/test-kernel"))
